...
const DECIMALS = 9
const INITIAL_SUPPLY = ethers.utils.parseUnits('22', 6 + DECIMALS)
...
async function setupContracts() {
  ...
  const factory = await ethers.getContractFactory('CEEU')
...
describe('CEEU:deployment', function () {
  it('check that deployment works', async () => {
    ...
    const upgraded = await upgrades.upgradeProxy(instance.address, CEEU)
...
describe('CEEU:Initialization', () => {
  before('setup UFragments contract', setupContracts)

  it('should transfer 22M uFragments to the deployer', async function () {
    expect(await uFragments.balanceOf(await deployer.getAddress())).to.eq(
      INITIAL_SUPPLY,
    )
  })

  it('should set the totalSupply to 22M', async function () {
  ...
  it('should set detailed ERC20 parameters', async function () {
    expect(await uFragments.name()).to.eq('Carbon Emissions of EU')
    expect(await uFragments.symbol()).to.eq('CEEU')
    expect(await uFragments.decimals()).to.eq(DECIMALS)
  })
})

describe('CEEU:setMonetaryPolicy', async () => {
...
describe('CEEU:setMonetaryPolicy:accessControl', async () => {
  let policy: Signer, policyAddress: string
...
describe('CEEU:Rebase:accessControl', async () => {
  let user: Signer, userAddress: string

  before('setup CEEU contract', async function () {
    await setupContracts()
    user = accounts[1]
...
describe('CEEU:Rebase:Expansion', async () => {
  ...
  before('setup CEEU contract', async function () {
    ...
    expect(await uFragments.scaledBalanceOf(await A.getAddress())).to.eq(
      '3947457587635779389439919943477996858634204022692291955890599250000000000',
    )
    expect(await uFragments.scaledBalanceOf(await B.getAddress())).to.eq(
      '1315819195878593129813306647825998952878068007564097318630199750000000000',
    )
  })
...
describe('CEEU:Rebase:Expansion', async function () {
  let policy: Signer
...
describe('CEEU:Rebase:NoChange', function () {
...
describe('CEEU:Rebase:Contraction', function () {
  // Rebase -5M (-10%), with starting balances A:750 and B:250.
  let A: Signer, B: Signer, policy: Signer
  const rebaseAmt = INITIAL_SUPPLY.div(10)
...
describe('CEEU:Transfer', function () {
  ...
  describe('Modifying blacklist address correctly', function () {
    ...
    it('address that was removed should not be in list', async function () {
      expect(
        await uFragments.isAddressBlacklisted(deployer.getAddress()),
      ).to.eq(false)
    })
  })
