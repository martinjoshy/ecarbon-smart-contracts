...
contract CEEUPolicy is Ownable {
    ...
    event LogRebase(
        ...
        uint256 avgTradingPrice,
        uint256 euaContractPrice,
        int256 requestedSupplyAdjustment,
        uint256 timestampSec
    );

    IUFragments public CEEU;
     ...
     */
    function initialize(address owner_, IUFragments ceeu_) public initializer {
     ...
     *      Where DeviationFromTargetRate is (euaContractPrice - avgTradingPrice) / avgTradingPrice
     */
    function rebase(uint256 avgTradingPrice, uint256 euaContractPrice) external onlyOrchestrator {
        ...
        if (avgTradingPrice > MAX_RATE) {
            avgTradingPrice = MAX_RATE;
        }

        int256 supplyDelta = computeSupplyDelta(avgTradingPrice, euaContractPrice);

        // Apply the Dampening factor.
        supplyDelta = supplyDelta.div(rebaseLag.toInt256Safe());

        if (supplyDelta > 0 && CEEU.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(CEEU.totalSupply())).toInt256Safe();
        }

        uint256 supplyAfterRebase = CEEU.rebase(epoch, supplyDelta);
        assert(supplyAfterRebase <= MAX_SUPPLY);
        emit LogRebase(epoch, avgTradingPrice, euaContractPrice, supplyDelta, block.timestamp);
    }
     ...
     *         and the targetRate.
     */
    function computeSupplyDelta(uint256 exchangeRate, uint256 targetRate)
        internal
        view
        returns (int256)
    {
        if (withinDeviationThreshold(exchangeRate, targetRate)) {
            return 0;
        }
        ...
        return
            CEEU
                .totalSupply()
                .toInt256Safe()
                .mul(exchangeRate.toInt256Safe().sub(targetRate.toInt256Safe()))
                .div(targetRate.toInt256Safe());
    }
}
